...
ggsave(paste0("article_supp/figs/lung_", i, ".svg"), height = 12, width = 15)
...
ggsave(paste0("article_supp/figs/prostate_", i, ".svg"), height = 5, width = 5)
...
ggsave(paste0("article_supp/figs/breast_", i, ".svg"), height = 5, width = 7)
...
## Data curated from TCGA:
### To see general dataset dimensions

readRDS("data/tcga_tumor_exp_luad.RDS")

readRDS("data/tcga_tumor_mirna_luad.RDS") %>% 
  distinct(barcode)
```
...
length(unique(c(filter(cerna_pattern_cancer, tissue == "LUAD")$geneA, filter(cerna_pattern_cancer, tissue == "LUAD")$geneB)))
       
type = "LUAD"

cancer <- cerna_pattern_cancer %>% 
  filter(tissue %in% selected_tissue) %>% 
  filter(tissue == type)
...
ggsave(paste0("article_supp/figs/luad_", i, ".png"), height = 4, width = 5)
...
readRDS("data/tcga_tumor_exp_prad.RDS")#501

readRDS("data/tcga_tumor_mirna_prad.RDS") %>% 
  distinct(barcode) #498
```



```{r, message=FALSE, warning=FALSE}
length(unique(c(filter(cerna_pattern_cancer, tissue == "PRAD")$geneA, filter(cerna_pattern_cancer, tissue == "PRAD")$geneB)))
type = "PRAD"
...
#127 mirna and 237 ceRNA and 1851 interaction
...
ggsave(paste0("article_supp/figs/prad_", i, ".png"), height = 4, width = 5)
...
readRDS("data/tcga_tumor_exp_brca.RDS")#1111

readRDS("data/tcga_tumor_mirna_brca.RDS") %>% 
  distinct(barcode) #1096
```
...
ggsave(paste0("article_supp/figs/brca_", i, ".png"), height = 4, width = 5)
...
tcga_project_normal <-readRDS("data/all_tcga_project_normal.RDS")
...
ggsave(paste0("article_supp/figs/luad_361_dea_kegg.svg"), plot = kegg, height = 7, width = 12)
ggsave(paste0("article_supp/figs/luad_361_dea_hallmark.svg"), plot = hallmark, height = 7, width = 12)
```
...
ggsave(paste0("article_supp/figs/luad_361_dea_genes.svg"), plot = graph_path, height = 7, width = 17)
```
...
ggsave(paste0("article_supp/LUAD/LUAD_vs_lung_", i, ".svg"), plot = graph3, height = 7, width = 5)
...
ggsave(paste0("article_supp/PRAD/PRAD_vs_prostate_", i, ".svg"), plot = graph3, height = 7, width = 5)
...
ggsave(paste0("article_supp/figs/brca_dea_hallmark.svg"), plot = hallmark,  height = 4, width = 12)
ggsave(paste0("article_supp/figs/brca_dea_kegg.svg"), plot = kegg, height = 4, width = 12)
...
ggsave(paste0("article_supp/figs/brca_dea_genes.svg"), plot = graph_path,  height = 7, width = 12)
```
...
ggsave(paste0("article_supp/BRCA/BRCA_vs_breast_", i, ".svg"), plot = graph3, height = 7, width = 5)
...
ggsave("article_supp/figs/all_three_samples_comparison.svg", height = 8, width = 8)
```
...
ggsave(filename = 'article_supp/figs/common_genes_go_terms.png', width = 10, height = 7)
```
...
ggsave("article_supp/LUAD/ceRNA_luad_commonn_three.svg", plot = heatmap,height = 3, width = 8)
```
...
ggsave("article_supp/PRAD/ceRNA_prad_commonn_three.svg", plot = heatmap,height = 3, width = 8)
```
...
ggsave("article_supp/BRCA/ceRNA_brca_commonn_three.svg", plot = heatmap,height = 3, width = 8)
```
...
```{r}
svg("article_supp/figs/common_ceRNA_overall_circlize.svg", height = 25, width = 25)
